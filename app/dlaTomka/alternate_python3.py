...
with open('miejscowosci.csv', newline='', encoding='utf-8') as f:
    reader = csv.reader(f)
    header = next(reader)
    for row in reader:
...
with open('miejscowosci_1.csv', newline='', encoding='utf-8') as f:
    reader = csv.reader(f)
    header = next(reader)
    for row in reader:
        #dane.append(dict(zip(header, row)))
...
print(json.dumps(list(d.values()), sort_keys=True, indent=4, separators=(',', ': ')))